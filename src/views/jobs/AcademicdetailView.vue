<template>
  <div>
    <form @submit.prevent="submitForm">
      ...
      <input
        v-model="formData.year_of_passing_college"
        type="number"
        required
      />

      <label>Percentage (College):</label>
      <input v-model="formData.percentage_college" type="number" required />
      ...
      <input v-model="formData.percentage_tenth" type="number" required />
      ...
      <input
        v-model="formData.year_of_passing_twelfth"
        type="number"
        required
      />

      <label>Percentage (12th):</label>
      <input v-model="formData.percentage_twelfth" type="number" required />
...
export default {
  ...
  methods: {
    submitForm() {
      axios
        .post(`${store.state.baseUrl}academicdetail`, this.formData)
          ...
          if (response.status === 200) {
            ...
            this.$router.push({
              ...
              params: {
                ...
                academic_id: response.data.academic_id,
            ...
            console.error(
              "Application submission failed with status code:",
              response.status
            );
          }
        })
