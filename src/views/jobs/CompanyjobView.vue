...
<h1>
 ...
 <button @click="openJobCreationModal" class="btn btn-success ms-auto" data-toggle="modal" data-target="#createJobModal">
...
<br>
              <button type="submit" class="btn btn-secondary">Create Job</button>
...
>
     ...
     <ag-grid-column headerName="Actions">
        <template>
          <button @click="editJob(params.data.id)">Edit</button>
          <button @click="deleteJob(params.data.id)">Delete</button>
        </template>
      </ag-grid-column>
</ag-grid-vue>
...
export default {
  ...
  methods: {
    ...
    fetchJobs() {
      ...
      axios.get(`${store.state.baseUrl}jobs`, {
        params: {
...
createJob() {
  ...
  axios.post(`${store.state.baseUrl}jobs`, {
    company_email: loggedInEmail,
